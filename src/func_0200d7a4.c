/* func_0200d7a4 at 0x0200d7a4
 * Checks if playerID == CURR_PLAYER_ID; if so, calls Camera::ChangeState
 * with a pointer to settingBehaviourTable[32] (offset 0x100 from base 0x0209B008).
 */
...
void func_0200d7a4(struct Camera *thiz, u8 playerID)
{
    if (playerID != CURR_PLAYER_ID)
        return;
    _ZN6Camera11ChangeStateEPNS_5StateE(thiz, &settingBehaviourTable[32]);
}
